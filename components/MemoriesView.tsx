...
const MemoriesView: React.FC<MemoriesViewProps> = ({ memories, onClose, onDeleteMemory, characterName, characterAvatar, userName }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 sm:p-8 animate-in fade-in">
      ...
      <div className="w-full max-w-5xl h-full overflow-y-auto custom-scrollbar pb-20 pt-10 px-4">
        
        {uniqueSortedMemories.length > 0 ? (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {uniqueSortedMemories.map((memory) => (
              <div 
                key={memory.id}
              ...
              >
                ...
                <div className="w-16 h-12 bg-[#e6dfd1] border border-[#dcd5c6] relative mb-3 shadow-inner flex items-center justify-center overflow-hidden">
                    ...
                    <i className="fa-solid fa-heart text-rose-400/50 text-xl"></i>
                </div>

                <p className="font-playfair-display text-lg font-bold text-[#5a4d41] mb-1">
                    {new Date(memory.timestamp).toLocaleDateString('vi-VN')}
                </p>
          ...
          <div className="flex flex-col items-center justify-center h-full text-center text-white/70">
            ...
            <p className="font-cormorant-garamond text-xl italic max-w-md">Chưa có dòng thư nào được viết. Hãy trò chuyện và lưu lại những khoảnh khắc đáng nhớ cùng {characterName} nhé.</p>
          </div>
        )}
      </div>
